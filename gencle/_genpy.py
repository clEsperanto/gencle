...
def _generate_function_wrapper(function_dict: dict, tier: int) -> str:
    ...
    return _wrapper_func_code.format(name=name, tier=tier, parameters_bindings=parameters_bindings).strip()
...
"""
    ...
    _wrapper_file_code = _wrapper_file_code.format(tier=tier, list_function_code=str_function_code)
...
def _convert_argument_from_cpp_to_python(parameter: dict) -> dict:
    ...
    return {"name": name, "type": type, "default_value": default_value, "description": description}
...
def _generate_function_docstring(function_dict: dict) -> str:
    ...
    for p in parameters:
        ...
        parameters_list.append(f"{param_name}: {param_type} {default_str}\n\t\t{description}")
    ...
    return _docstring_code.format(brief_str=brief_str, parameters_str=parameters_str, return_str=return_str, references_str=references_str)
...
"""
    ...
    for p in function_dict['parameters']:
        ...
        arguments_list.append(param_name if param_type not in ["int", "float", "str"] else f"{param_type}({param_name})")
    ...
    return _python_func_code.format(decorator=decorator, function_name=function_name, python_parameters_str=python_parameters_str, return_type=return_type, docstring_str=_docstring_str, arguments_str=arguments_str, deprecation_warning=deprecation_warning).strip()
...
"""
    ...
    _python_code = _python_code.format(tier=tier, python_functions_str=python_functions_str)
